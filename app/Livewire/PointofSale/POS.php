...
class POS extends Component
{
    ...
    public function checkout(): void
    {
        ...
        if (! $isCredit && (float) $this->paid_amount <= 0) {
            $this->notify('danger', 'No Tendered Amount', 'Please enter the tendered amount before checking out.');
            return;
        }

        if (! $isCredit && (float) $this->paid_amount < $this->subtotal) {
            $this->notify('danger', 'Insufficient Payment', 'Tendered amount is less than the total.');
            return;
        }
