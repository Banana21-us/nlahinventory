...
class PayoffManagement extends Component
{
    ...
    private function checkOverlap(?int $excludeId = null): bool
    {
        ...
        $poQuery = PayoffApplication::where('user_id', $uid)
            ->where('status', '!=', 'rejected')
            ->where('start_datetime', '<', $end)
            ->where('end_datetime', '>', $start);

        if ($excludeId) {
            $poQuery->where('id', '!=', $excludeId);
        }
        ...
        return OvertimeApplication::where('user_id', $uid)
            ->where('status', '!=', 'rejected')
            ->where('start_datetime', '<', $end)
            ->where('end_datetime', '>', $start)
            ->exists();
    ...
    public function save(): void
    {
        ...
        if ($this->checkOverlap()) {
            $this->addError('start_datetime', 'This time period overlaps with an existing pay-off or overtime application.');
