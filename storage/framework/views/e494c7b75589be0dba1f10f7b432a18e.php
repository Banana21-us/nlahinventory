...
<div class="max-w-7xl mx-auto py-8 px-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-lg shadow-md p-3"><div class="flex justify-between"><div><p class="text-xs text-gray-500">Needs Repair</p><p class="text-2xl font-bold text-amber-600"><?php echo e($assets->where('status', 'maintenance')->count()); ?></p></div><div class="p-2 bg-amber-100 rounded-full"><svg class="w-4 h-4 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg></div></div></div>
        <div class="bg-white rounded-lg shadow-md p-3"><div class="flex justify-between"><div><p class="text-xs text-gray-500">Poor Condition</p><p class="text-2xl font-bold text-red-600"><?php echo e($assets->where('condition_status', 'poor')->count()); ?></p></div><div class="p-2 bg-red-100 rounded-full"><svg class="w-4 h-4 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></div></div></div>
        <div class="bg-white rounded-lg shadow-md p-3"><div class="flex justify-between"><div><p class="text-xs text-gray-500">Fair Condition</p><p class="text-2xl font-bold text-yellow-600"><?php echo e($assets->where('condition_status', 'fair')->count()); ?></p></div><div class="p-2 bg-yellow-100 rounded-full"><svg class="w-4 h-4 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg></div></div></div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $assets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $asset): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
            <?php $needRepair = $asset->status === 'maintenance'; ?>
            <div class="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden cursor-pointer hover:shadow-lg transition <?php echo e($needRepair ? 'border-l-4 border-l-red-500' : ($asset->condition_status === 'poor' ? 'border-l-4 border-l-amber-500' : 'border-l-4 border-l-yellow-500')); ?>" wire:click="showDetails(<?php echo e($asset->id); ?>)">
                <div class="h-32 bg-gray-100">
                    ...
                    <div class="absolute top-2 right-2"><span class="text-white text-xs font-bold px-2 py-1 rounded-full <?php echo e($needRepair ? 'bg-red-600' : ($asset->condition_status === 'poor' ? 'bg-amber-600' : 'bg-yellow-600')); ?>"><?php echo e($needRepair ? 'Needs Repair' : ucfirst($asset->condition_status)); ?></span></div>
                </div>
                <div class="p-3"><p class="font-bold text-gray-900 text-sm truncate"><?php echo e($asset->asset_code); ?></p><p class="font-semibold text-gray-800 text-xs truncate"><?php echo e($asset->name); ?></p></div>
            </div>
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
    </div>
    <div class="mt-6"><?php echo e($assets->links()); ?></div>
    <?php else: ?>
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div class="relative bg-white rounded-xl shadow-xl sm:w-full sm:max-w-2xl">
                ...
                <div class="px-6 py-5"><div class="flex flex-col md:flex-row gap-6">
                    <div class="md:w-1/2 bg-gray-50 rounded-lg p-4 flex justify-center"><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($selectedAsset->image && Storage::disk('public')->exists($selectedAsset->image)): ?><img src="<?php echo e(Storage::url($selectedAsset->image)); ?>" class="max-w-full h-48 object-cover rounded-lg"><?php else: ?><div class="w-full h-48 bg-gray-200 rounded-lg flex items-center justify-center"><svg class="w-16 h-16 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg></div><?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?></div>
                    <div class="md:w-1/2 space-y-3"><div><p class="text-xs font-bold text-gray-400">Asset Name</p><p class="font-semibold"><?php echo e($selectedAsset->name); ?></p></div><div><p class="text-xs font-bold text-gray-400">Asset Code</p><p><?php echo e($selectedAsset->asset_code); ?></p></div><div class="grid grid-cols-2 gap-3"><div><p class="text-xs font-bold text-gray-400">Brand</p><p><?php echo e($selectedAsset->brand ?: 'N/A'); ?></p></div><div><p class="text-xs font-bold text-gray-400">Model</p><p><?php echo e($selectedAsset->model ?: 'N/A'); ?></p></div></div><div><p class="text-xs font-bold text-gray-400">Serial Number</p><p><?php echo e($selectedAsset->serial_number ?: 'N/A'); ?></p></div><div class="grid grid-cols-2 gap-3"><div><p class="text-xs font-bold text-gray-400">Status</p><span class="px-2 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-700"><?php echo e(ucfirst(str_replace('_', ' ', $selectedAsset->status))); ?></span></div><div><p class="text-xs font-bold text-gray-400">Condition</p><span class="px-2 py-1 rounded-full text-xs font-semibold <?php echo e($selectedAsset->condition_status === 'poor' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'); ?>"><?php echo e(ucfirst($selectedAsset->condition_status)); ?></span></div></div><div><p class="text-xs font-bold text-gray-400">Location</p><p><?php echo e($selectedAsset->location->name ?? 'N/A'); ?></p></div><div><p class="text-xs font-bold text-gray-400">Notes</p><p class="text-sm"><?php echo e($selectedAsset->notes ?: 'No notes.'); ?></p></div></div>
                </div></div>
                <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3 rounded-b-xl"><button wire:click="closeDetailsModal" class="px-4 py-2 text-sm font-semibold text-gray-700">Close</button><button wire:click="openRepairModal(<?php echo e($selectedAsset->id); ?>)" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-bold">Complete Repair</button></div>
            </div>
        </div>
    </div>
    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
            ...
            <div class="relative bg-white rounded-xl shadow-xl sm:w-full sm:max-w-lg">
                <form wire:submit.prevent="completeRepair">
                    ...
                    <div class="px-6 py-5 space-y-4">
                        <div><label class="block text-xs font-bold text-gray-500 mb-1">Issue / Problem *</label><textarea wire:model="repair_issue" rows="3" class="w-full rounded-md border p-2" placeholder="Describe what was repaired..."></textarea><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__errorArgs = ['repair_issue'];
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?></div>
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(session()->has('message')): ?><div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)" class="fixed bottom-5 right-5 bg-green-500 text-white px-4 py-2 rounded-lg shadow-lg text-sm"><?php echo e(session('message')); ?></div><?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
</div><?php /**PATH C:\Users\JOHNPAUL\OneDrive\Documents\GitHub\nlahinventory\resources\views/pages/Repair/repair.blade.php ENDPATH**/ ?>