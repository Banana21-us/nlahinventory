    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <h1 class="text-xl font-bold text-gray-800 leading-tight">Holidays</h1>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white shadow-md rounded-lg border border-gray-200 overflow-hidden"
        ...
        <button @click="open = !open"
            ...
            <div class="flex items-center">
                ...
                <h2 class="text-lg font-bold text-gray-800">Holiday Entry</h2>
            </div>
            <span class="text-sm font-medium brand-text-primary" x-text="open ? 'Minimize' : 'Add New Holiday'"></span>
        </button>
        ...
        <div x-show="open" x-collapse class="p-6 border-t border-gray-100 bg-gray-50/30">
            <form wire:submit.prevent="save" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wide text-gray-500 mb-1">Holiday Name *</label>
                    <input type="text" wire:model="name" placeholder="e.g. New Year's Day"
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                ...
                <div class="flex items-center gap-3 pt-5">
                    <input type="checkbox" wire:model="is_recurring" id="is_recurring_add" class="rounded border-gray-300 brand-text-primary"/>
                    <label for="is_recurring_add" class="text-sm font-medium text-gray-700">Recurring annually</label>
                </div>
        ...
        <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex flex-wrap gap-3 justify-between items-center">
            <div class="flex items-center gap-3">
                ...
                <span class="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full font-medium">
                    <?php echo e($holidays->count()); ?> <?php echo e(Str::plural('holiday', $holidays->count())); ?>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $holidays; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $holiday): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                        ...
                        <tr class="brand-row-hover transition-colors">
                            <td class="px-6 py-4 text-sm font-semibold text-gray-800"><?php echo e($holiday->date->format('M d, Y')); ?></td>
                            <td class="px-6 py-4 text-sm font-bold text-gray-900"><?php echo e($holiday->name); ?></td>
                            <td class="px-6 py-4">
                                <span class="px-2.5 py-0.5 inline-flex text-xs leading-5 font-bold rounded-full <?php echo e($typeBadge[0]); ?>">
                                    <?php echo e($typeBadge[1]); ?>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-500"><?php echo e($holiday->remarks ?? '—'); ?></td>
                            <td class="px-6 py-4 text-right text-sm font-medium space-x-2">
                                <button wire:click="edit(<?php echo e($holiday->id); ?>)" class="brand-edit-btn rounded-md px-2.5 py-1.5 text-sm font-semibold shadow-sm transition-colors">Edit</button>
                                <button wire:click="confirmDelete(<?php echo e($holiday->id); ?>)" class="text-red-500 hover:text-red-700 font-semibold transition-colors">Delete</button>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-14 text-center">
                                <div class="flex flex-col items-center text-gray-400">
                                    ...
                                    <p class="text-sm font-medium"><?php echo e($search ? 'No holidays match your search.' : 'No holidays found.'); ?></p>
                                    <p class="text-xs mt-1"><?php echo e($search ? 'Try a different keyword.' : 'Click "Add New Holiday" above to get started.'); ?></p>
                                </div>
                            </td>
                        </tr>
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                ...
                <div class="relative bg-white rounded-xl shadow-xl sm:w-full sm:max-w-lg">
                    <div class="px-6 pt-6 pb-4">
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                <h3 class="text-base font-bold text-gray-900">Delete Holiday</h3>
                                <p class="mt-1.5 text-sm text-gray-500">Are you sure you want to delete this holiday? This cannot be undone.</p>
                            </div>
                        </div>
                    </div>
                    <div class="bg-gray-50 px-6 py-4 flex flex-row-reverse gap-3 rounded-b-xl">
                        <button wire:click="delete" class="inline-flex justify-center rounded-lg px-4 py-2 text-sm font-bold text-white bg-red-600 hover:bg-red-500 transition-colors active:scale-95">Delete Permanently</button>
                    ...
                    <form wire:submit.prevent="update">
                        <div class="px-6 pt-6 pb-4">
                            <div class="flex items-center mb-5 pb-4 border-b border-gray-100">
                                ...
                                <h3 class="text-xl font-bold text-gray-900">Update Holiday</h3>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-xs font-bold uppercase tracking-wide text-gray-500 mb-1">Holiday Name *</label>
                                    <input type="text" wire:model="name" class="brand-focus block w-full rounded-md border border-gray-300 shadow-sm sm:text-sm p-2"/>
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__errorArgs = ['name'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                ...
                                <div>
                                    <label class="block text-xs font-bold uppercase tracking-wide text-gray-500 mb-1">Date *</label>
                                    <input type="date" wire:model="date" class="brand-focus block w-full rounded-md border border-gray-300 shadow-sm sm:text-sm p-2"/>
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__errorArgs = ['date'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                ...
                                <div>
                                    <label class="block text-xs font-bold uppercase tracking-wide text-gray-500 mb-1">Remarks</label>
                                    <input type="text" wire:model="remarks" class="brand-focus block w-full rounded-md border border-gray-300 shadow-sm sm:text-sm p-2"/>
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__errorArgs = ['remarks'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                ...
                                <div class="flex items-center gap-3">
                                    <input type="checkbox" wire:model="is_recurring" id="is_recurring_edit" class="rounded border-gray-300"/>
                                    <label for="is_recurring_edit" class="text-sm font-medium text-gray-700">Recurring annually</label>
                                </div>
                            </div>
                        </div>
                        <div class="bg-gray-50 px-6 py-4 flex flex-row-reverse gap-3 rounded-b-xl">
...
<?php /**PATH C:\Users\admin\Documents\GitHub\nlah\resources\views/pages/HR/holiday-management.blade.php ENDPATH**/ ?>