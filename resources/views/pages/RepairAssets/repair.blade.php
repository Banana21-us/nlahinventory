        ...
        <div class="bg-white rounded-lg shadow-md border border-gray-200 p-3">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-amber-600">{{ $assets->where('status', 'maintenance')->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-md border border-gray-200 p-3">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-red-600">{{ $assets->where('condition_status', 'poor')->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-md border border-gray-200 p-3">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-yellow-600">{{ $assets->where('condition_status', 'fair')->count() }}</p>
                </div>
    ...
    @if($assets && $assets->count() > 0)
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            @foreach($assets as $asset)
                ...
                @endphp
                <div class="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden hover:shadow-lg transition-all duration-300 cursor-pointer {{ $priorityClass }}" wire:click="showDetails({{ $asset->id }})">
                    <!-- Card Image -->
                    ...
                    <div class="p-3">
                        ...
                        <div class="mt-2 text-xs">
                            <div class="flex justify-between">
                                ...
                                @endphp
                                <span class="font-semibold {{ $conditionColor }}">{{ ucfirst($asset->condition_status) }}</span>
                            </div>
                            <div class="flex justify-between mt-1">
                                <span class="text-gray-500">Status:</span>
                                <span class="font-semibold">{{ ucfirst(str_replace('_', ' ', $asset->status)) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-md border border-gray-200 p-12 text-center">
            <div class="flex flex-col items-center">
                ...
                <p class="text-sm text-gray-500 max-w-md">
                    {{ $search ? 'No assets match your search criteria.' : 'All assets are in good condition. No repairs needed at this time.' }}
                </p>
                @if($search)
            ...
            <div class="flex min-h-full items-center justify-center p-4">
                <div class="relative transform overflow-hidden rounded-xl bg-white text-left shadow-xl sm:w-full sm:max-w-2xl w-full max-w-2xl">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-amber-200">{{ $selectedAsset->asset_code }}</p>
                                </div>
                            </div>
                        ...
                        <div class="flex flex-col md:flex-row gap-6">
                            ...
                            <div class="md:w-1/2 bg-gray-50 rounded-lg p-4 flex items-center justify-center">
                                @if($selectedAsset->image && Storage::disk('public')->exists($selectedAsset->image))
                                    <img src="{{ Storage::url($selectedAsset->image) }}" class="w-full max-w-sm object-cover rounded-lg shadow-md">
                                @else
                            ...
                            <div class="md:w-1/2 space-y-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-700">{{ $selectedAsset->asset_code }}</p>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold {{ $conditionColor }}">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-700">{{ $selectedAsset->location->name ?? 'N/A' }}</p>
                                </div>

                                <div>
                                    <p class="text-xs font-bold uppercase tracking-wide text-gray-400">Notes</p>
                                    <p class="text-gray-600 text-sm">{{ $selectedAsset->notes ?: 'No notes available.' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3 rounded-b-xl">
                        <button 
                            type="button" 
                            wire:click="openRepairModal({{ $selectedAsset->id }})" 
                    ...
                    <form wire:submit.prevent="completeRepair">
                        ...
                        <div class="px-6 py-5 space-y-4">
                            <div>
                                ...
                                @error('repair_issue') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                            </div>
                            ...
                            <div>
                                ...
                                @error('repair_cost') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                            </div>
