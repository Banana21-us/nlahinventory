...
<div
    ...
    <div
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="flex flex-col sm:flex-row gap-4">
                ...
                <div
                    ...
                    @foreach (['all' => 'All', 'meals' => '🍱', 'drinks' => '🥤', 'snacks' => '🍟']
                    as $key => $label)
                    <button
                        type="button"
        ...
        @endif @if(session()->has('checkout_success'))
        <div
            x-data="{ show: true }"
            x-show="show"
        ...
        <div class="flex-1 overflow-y-auto px-6 pb-6 custom-scrollbar">
            <div
                ...
                <button
                    ...
                    @disabled($this->extraUtensilsDisabled)="@disabled($this->extraUtensilsDisabled)"
                ...
                <button
                    ...
                    @disabled($this->budgetMealDisabled)="@disabled($this->budgetMealDisabled)"
                    class="group text-left border rounded-xl overflow-hidden transition-all
                ...
                @forelse ($this->filteredItems as $item) @php $imagePath = $item->image ?
                ...
                <button
                    ...
                    <div class="p-2">
                        <h3
                            class="text-xs font-bold text-slate-800 line-clamp-1 capitalize leading-tight">{{ $item->name }}</h3>
        ...
        <div class="flex-1 overflow-y-auto px-6 space-y-2 min-h-0">
            ...
            <div
                ...
                <div class="flex-1 min-w-0">
                    ...
                    @endif
                    <p class="text-[11px] font-medium text-slate-400 mt-0.5">₱{{ number_format($cartItem['price'], 2) }}
                        / unit</p>
                </div>
        ...
        {{ (float)$sc->balance >= $this->subtotal
            ...
            : 'bg-red-50 border-red-200' }}">
                ...
                <div class="flex items-center justify-between text-[11px]">
                    ...
                    <span
                        class="font-bold {{ $remaining >= 0 ? 'text-slate-600' : 'text-red-600' }}">
                        ₱{{ number_format(max(0, $remaining), 2) }}
                        @if($remaining < 0)
                        <span class="text-[10px] font-semibold text-red-500">(₱{{ number_format(abs($remaining), 2) }}
                            over)</span>
                        @endif
                    </span>
                </div>
                ...
                @endif @if((float)$sc->balance < $this->subtotal)
            ...
            <div class="p-3.5 bg-slate-50 rounded-2xl border border-slate-200 space-y-2">
                ...
                <div class="flex justify-between text-xs font-medium text-slate-500">
                    <span>Change</span>
                    <span
                        class="font-bold {{ $this->change >= 0 ? 'text-emerald-600' : 'text-red-500' }}">
            ...
            <div class="space-y-4 px-6 py-5">
                ...
                <div>
                    ...
                    <select
                        ...
                        <option value="{{ $rice->id }}">{{ $rice->name }}
                            - ₱{{ number_format($rice->price, 2) }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select
                        ...
                        <option value="{{ $meal->id }}">{{ $meal->name }}
                            - ₱{{ number_format($meal->price, 2) }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select
                        ...
                        <option value="{{ $utensil->id }}">{{ $utensil->name }}
                            - ₱{{ number_format($utensil->price, 2) }}</option>
                        @endforeach
                    </select>
                    <div class="space-y-2 rounded-xl border border-slate-200 bg-slate-50 p-3">
                        @foreach($this->budgetMealUtensilOptions as $utensil)
    ...
    ══════════════════════════════════════════════════
--}}
    @if($showCreditConfirm && $this->selectedCustomer) @php $sc =
    $this->selectedCustomer; @endphp
        ...
        <div
            ...
            <div class="p-6">
                ...
                <p class="text-xs text-stone-400 leading-relaxed mb-5">
                    ...
                    and
                    <strong class="text-red-500">₱{{ number_format($creditShortfall, 2) }}</strong>
