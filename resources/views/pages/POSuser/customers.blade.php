<div class="max-w-7xl mx-auto py-8 px-4">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
            ...
            <p class="text-2xl font-extrabold text-gray-900">{{ number_format($this->summary['total']) }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
            <p class="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-1">Active</p>
            <p class="text-2xl font-extrabold text-green-600">{{ number_format($this->summary['active']) }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
            <p class="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-1">Inactive</p>
            <p class="text-2xl font-extrabold text-gray-400">{{ number_format($this->summary['inactive']) }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
            <p class="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-1">With Credit</p>
            <p class="text-2xl font-extrabold text-red-500">{{ number_format($this->summary['credited']) }}</p>
        </div>
    </div>
        ...
        <div x-show="open" x-collapse
            ...
            <form wire:submit.prevent="save" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    @error('charges') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                </div>
            ...
            <table class="w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse($this->customers as $customer)
                        ...
                        @endphp
                        <tr class="hover:bg-amber-50/40 transition-colors {{ $hasCharges ? 'bg-red-50/20' : '' }}">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full flex items-center justify-center shrink-0 font-bold text-sm
                                        {{ $hasCharges ? 'bg-red-100 text-red-600' : 'bg-amber-100 text-amber-700' }}">
                                        {{ strtoupper(substr($customer->name, 0, 1)) }}
                                    </div>
                                    <div>
                                        <span class="text-sm font-bold text-gray-900">{{ $customer->name }}</span>
                                        @php $txCount = $customer->sales()->count(); @endphp
                                        @if($txCount > 0)
                                            <p class="text-[11px] text-gray-400 font-medium">{{ $txCount }} {{ Str::plural('transaction', $txCount) }}</p>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <span class="text-sm font-bold {{ $balance > 0 ? 'text-gray-800' : 'text-gray-400' }}">
                            ...
                            <td class="px-6 py-4">
                                @if($hasCharges)
                                    <div class="flex items-center gap-2">
                                        <span class="text-sm font-bold text-red-600">₱{{ number_format($charges, 2) }}</span>
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-14 text-center">
                                <div class="flex flex-col items-center text-gray-400">
                                    ...
                                    <p class="text-sm font-medium">{{ ($search || $filterStatus) ? 'No customers match your filters.' : 'No customers yet.' }}</p>
                                    @if($search || $filterStatus)
    ...
    ═══════════════════════════════════════════ --}}
    @if($showHistory && $this->historyCustomer)
        @php
                    ...
                    <div class="px-6 pt-5 pb-4 border-b border-gray-100">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full flex items-center justify-center font-black text-base
                                    {{ (float)$hc->charges > 0 ? 'bg-red-100 text-red-600' : 'bg-amber-100 text-amber-700' }}">
                                    {{ strtoupper(substr($hc->name, 0, 1)) }}
                                </div>
                                <div>
                                    <h3 class="text-lg font-black text-gray-900 leading-tight">{{ $hc->name }}</h3>
                                    <p class="text-xs text-gray-400">{{ $hc->phone ?? 'No phone' }} · {{ ucfirst($hc->status) }}</p>
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-3 gap-3 mt-4">
                            <div class="bg-stone-50 rounded-xl p-3 border border-stone-100">
                                ...
                                <p class="text-xl font-black text-stone-900">{{ $history->count() }}</p>
                            </div>
                            <div class="bg-amber-50 rounded-xl p-3 border border-amber-100">
                                <p class="text-[10px] font-bold uppercase tracking-wider text-amber-500 mb-0.5">Total Spent</p>
                                <p class="text-xl font-black text-stone-900">₱{{ number_format($totalSpent) }}</p>
                            </div>
                            <div class="rounded-xl p-3 border {{ (float)$hc->charges > 0 ? 'bg-red-50 border-red-100' : 'bg-emerald-50 border-emerald-100' }}">
                                <p class="text-[10px] font-bold uppercase tracking-wider mb-0.5 {{ (float)$hc->charges > 0 ? 'text-red-400' : 'text-emerald-500' }}">
                                    {{ (float)$hc->charges > 0 ? 'Outstanding' : 'Balance' }}
                                </p>
                                <p class="text-xl font-black {{ (float)$hc->charges > 0 ? 'text-red-600' : 'text-stone-900' }}">
                                    ₱{{ number_format((float)$hc->charges > 0 ? $hc->charges : $hc->balance, 2) }}
                                </p>
                            </div>
                        </div>
                    </div>
                                        ...
                                        <div class="flex items-start justify-between gap-4">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <div class="flex items-center gap-2 mb-1.5">
                                                    <span class="text-xs font-bold text-gray-500">#{{ $sale->id }}</span>
                                                    <span class="text-xs text-gray-400">{{ $sale->created_at->format('M d, Y · h:i A') }}</span>
                                                    <span class="text-[10px] text-gray-300">{{ $sale->created_at->diffForHumans() }}</span>
                                                </div>
                                                {{-- Items list --}}
                                                <div class="flex flex-wrap gap-1.5">
                                                    @foreach($sale->saleItems as $si)
                                                        <span class="inline-flex items-center gap-1 px-2 py-0.5 bg-gray-100 rounded-full text-[11px] font-medium text-gray-600">
                                                            {{ $si->item?->name ?? '?' }}
                                                            <span class="text-gray-400">×{{ $si->quantity }}</span>
                                                        </span>
                                                    @endforeach
                                                </div>
                                            </div>
                                            ...
                                            <div class="text-right shrink-0">
                                                <p class="text-base font-black text-gray-900">₱{{ number_format($sale->total) }}</p>
                                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold mt-1 {{ $mc }}">
                                                    {{ $sale->payment_method ?? '—' }}
                                                </span>
                                            </div>
                                        </div>
