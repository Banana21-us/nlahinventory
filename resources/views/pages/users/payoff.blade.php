    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 mt-0.5">Redeem earned pay-off hours as cash or vacation leave</p>
            </div>
        </div>
    </div>
                ...
                <div class="mb-4">
                    ...
                    <div class="flex gap-4">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" wire:model="redemption_type" value="cash"
                                class="w-4 h-4 accent-[#015581]"/>
                            <span class="text-sm font-semibold text-gray-700">Cash</span>
                            <span class="text-xs text-gray-400">(Dept Head → HR → Accounting)</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" wire:model="redemption_type" value="leave"
                                class="w-4 h-4 accent-[#015581]"/>
                            <span class="text-sm font-semibold text-gray-700">Vacation Leave</span>
                            <span class="text-xs text-gray-400">(Dept Head → HR)</span>
                        </label>
                    </div>
                    @error('redemption_type') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                </div>
                    ...
                    @forelse($applications as $app)
                        @php
                            $statusStyles = match($app->status) {
                                ...
                                'dept_approved' => 'background-color:#ede9fe;color:#5b21b6;border:1px solid #c4b5fd;',
                                default         => 'background-color:#fef3c7;color:#92400e;border:1px solid #fcd34d;',
                            };
                            $statusLabel = match($app->status) {
                                'dept_approved' => 'Pending HR',
                                'hr_approved'   => $app->redemption_type === 'cash' ? 'Pending Accounting' : 'Approved',
                        ...
                        <tr class="brand-row-hover transition-colors">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600">
                                {{ $app->accountingApprover?->name ?? $app->hrApprover?->name ?? $app->deptHeadApprover?->name ?? '—' }}
                            </td>
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center text-gray-400">
                                    ...
                                    <p class="text-xs mt-1">Click "Apply Now" above to submit as cash or leave.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                    ...
                    <form wire:submit.prevent="update">
                        ...
                        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="md:col-span-2">
                                ...
                                <div class="flex gap-4">
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" wire:model="redemption_type" value="cash" class="w-4 h-4 accent-[#015581]"/>
                                        <span class="text-sm font-semibold text-gray-700">Cash</span>
                                    </label>
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" wire:model="redemption_type" value="leave" class="w-4 h-4 accent-[#015581]"/>
                                        <span class="text-sm font-semibold text-gray-700">Vacation Leave</span>
                                    </label>
                                </div>
                            </div>
