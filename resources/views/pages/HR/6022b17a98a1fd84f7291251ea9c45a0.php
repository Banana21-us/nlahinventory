...
<div class="max-w-7xl mx-auto py-8 px-4">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-lg shadow-md p-3">
            <div class="flex justify-between">
                <div><p class="text-xs text-gray-500">Total Assets</p><p class="text-xl font-bold"><?php echo e($assets->total()); ?></p></div>
        ...
        <div class="bg-white rounded-lg shadow-md p-3">
            <div class="flex justify-between">
                <div><p class="text-xs text-gray-500">In Use</p><p class="text-xl font-bold text-blue-600"><?php echo e($assets->where('status', 'in_use')->count()); ?></p></div>
        ...
        <div class="bg-white rounded-lg shadow-md p-3">
            <div class="flex justify-between">
                <div><p class="text-xs text-gray-500">Maintenance</p><p class="text-xl font-bold text-amber-600"><?php echo e($assets->where('status', 'maintenance')->count()); ?></p></div>
        ...
        <div class="bg-white rounded-lg shadow-md p-3">
            <div class="flex justify-between">
                <div><p class="text-xs text-gray-500">Retired</p><p class="text-xl font-bold text-red-600"><?php echo e($assets->where('status', 'retired')->count()); ?></p></div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $assets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $asset): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
            <div class="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden cursor-pointer hover:shadow-lg transition" wire:click="showDetails(<?php echo e($asset->id); ?>)">
                ...
                <div class="p-3">
                    ...
                    <div class="mt-2">
                        <?php $statusColor = match($asset->status) { 'in_use' => 'bg-blue-100 text-blue-700', 'maintenance' => 'bg-amber-100 text-amber-700', 'retired' => 'bg-rose-100 text-rose-700', default => 'bg-gray-100 text-gray-700' }; ?>
                        <span class="px-2 py-0.5 rounded-full text-xs font-semibold <?php echo e($statusColor); ?>"><?php echo e(ucfirst(str_replace('_', ' ', $asset->status))); ?></span>
                    </div>
                </div>
            </div>
                    ...
                    <div class="flex flex-col md:flex-row gap-6">
                        <div class="md:w-1/2 bg-gray-50 rounded-lg p-4 flex justify-center">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($selectedAsset->image && Storage::disk('public')->exists($selectedAsset->image)): ?>
                                <img src="<?php echo e(Storage::url($selectedAsset->image)); ?>" class="max-w-full h-48 object-cover rounded-lg">
                            <?php else: ?>
                        ...
                        <div class="md:w-1/2 space-y-3">
                            <div><p class="text-xs font-bold text-gray-400">Asset Name</p><p class="font-semibold"><?php echo e($selectedAsset->name); ?></p></div>
                            <div><p class="text-xs font-bold text-gray-400">Asset Code</p><p><?php echo e($selectedAsset->asset_code); ?></p></div>
                            <div class="grid grid-cols-2 gap-3"><div><p class="text-xs font-bold text-gray-400">Brand</p><p><?php echo e($selectedAsset->brand ?: 'N/A'); ?></p></div><div><p class="text-xs font-bold text-gray-400">Model</p><p><?php echo e($selectedAsset->model ?: 'N/A'); ?></p></div></div>
                            <div><p class="text-xs font-bold text-gray-400">Serial Number</p><p><?php echo e($selectedAsset->serial_number ?: 'N/A'); ?></p></div>
                            <div class="grid grid-cols-2 gap-3">
                                <div><p class="text-xs font-bold text-gray-400">Status</p><span class="px-2 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-700"><?php echo e(ucfirst(str_replace('_', ' ', $selectedAsset->status))); ?></span></div>
                                <div><p class="text-xs font-bold text-gray-400">Condition</p><span class="px-2 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-700"><?php echo e(ucfirst($selectedAsset->condition_status)); ?></span></div>
                            </div>
                            <div><p class="text-xs font-bold text-gray-400">Location</p><p><?php echo e($selectedAsset->location->name ?? 'N/A'); ?></p></div>
                            <div><p class="text-xs font-bold text-gray-400">Notes</p><p class="text-sm"><?php echo e($selectedAsset->notes ?: 'No notes.'); ?></p></div>
                        </div>
                    </div>
                </div>
                <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3 rounded-b-xl">
                    <button wire:click="closeDetailsModal" class="px-4 py-2 text-sm font-semibold text-gray-700">Close</button>
                    <button wire:click="openUpdateModal(<?php echo e($selectedAsset->id); ?>)" class="bg-amber-600 hover:bg-amber-700 text-white px-4 py-2 rounded-lg text-sm font-bold">Update Status</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(session()->has('error')): ?><div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)" class="fixed bottom-5 right-5 bg-red-500 text-white px-4 py-2 rounded-lg shadow-lg text-sm"><?php echo e(session('error')); ?></div><?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
