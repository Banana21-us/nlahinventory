...
<style>
    ...
    <div class="px-8 pt-8 pb-0 flex items-end justify-between">
        ...
        <div class="text-right text-xs text-gray-400">
            <p class="font-bold text-gray-600">{{ now()->format('F Y') }}</p>
    ...
    <div class="px-8 pb-10 flex flex-col gap-6">
        ...
        {{-- ══════════════════════════════════════════════════════════════════
             ROW 1 — SHIFT DIFFERENTIAL CALCULATOR
        ══════════════════════════════════════════════════════════════════ --}}
            ...
            <div class="s-body">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div class="rounded-xl brand-bg-p-lt border border-blue-100 p-4 text-center">
                        ...
                        <p class="text-2xl font-black text-gray-900">{{ number_format($regularHours, 1) }}</p>
                    ...
                    <div class="rounded-xl brand-bg-t-lt border border-teal-100 p-4 text-center">
                        ...
                        <p class="text-2xl font-black text-gray-900">{{ number_format($nightDiffHours, 1) }}</p>
                        <p class="text-[11px] text-gray-400 mt-0.5">
                            @if($shiftDayType === 'regular') ×1.10 premium
                            @elseif($shiftDayType === 'special') ×1.375 premium
                            @else ×2.10 premium
                            @endif
                        </p>
                    </div>
                    ...
                    <div class="rounded-xl brand-bg-a-lt border border-yellow-100 p-4 text-center">
                        ...
                        <p class="text-2xl font-black text-gray-900">{{ number_format($overtimeHours, 1) }}</p>
                        <p class="text-[11px] text-gray-400 mt-0.5">
                            @if($shiftDayType === 'regular') ×1.25 OT
                            @elseif($shiftDayType === 'special') ×1.69 OT
                            @else ×2.60 OT
                            @endif
                        </p>
                    </div>
                    ...
                    {{-- Gross Pay --}}
                    <div class="rounded-xl border p-4 text-center {{ $grossPay > 0 ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200' }}">
                        <p class="text-[10px] font-black uppercase tracking-widest text-green-700 mb-1">Estimated Gross</p>
                        <p class="text-2xl font-black text-gray-900">₱{{ number_format($grossPay, 2) }}</p>
        ...
        {{-- ══════════════════════════════════════════════════════════════════
             ROW 2 — GOVT CONTRIBUTIONS · OT BURN RATE · 13TH MONTH
        ══════════════════════════════════════════════════════════════════ --}}
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">

            {{-- Government Contribution Status --}}
            <div class="s-card">
                <div class="s-head">
                ...
                <div class="s-body">
                    ...
                    <p class="text-xs text-gray-400 mb-4">{{ $contributions['total'] }} employee(s) in employment records</p>

                    @foreach($contribs as $c)
                    @php $pct = round(($c['count'] / $total) * 100); @endphp
                    <div class="contrib-row">
                        <div class="flex items-center gap-2.5">
                            <span class="text-lg leading-none">{{ $c['icon'] }}</span>
                            <div>
                                <p class="text-sm font-bold text-gray-800">{{ $c['label'] }}</p>
                                <p class="text-[10px] text-gray-400">{{ $c['count'] }} / {{ $contributions['total'] }} on file</p>
                            </div>
                        </div>
                        <div class="text-right min-w-[80px]">
                            <p class="text-sm font-black" style="color:{{ $c['color'] }};">{{ $pct }}%</p>
                            <div class="gauge-track mt-1 w-20">
                                <div class="gauge-fill" style="width:{{ $pct }}%;background:{{ $c['color'] }};"></div>
                            </div>
                        </div>
                    </div>
                    @endforeach
            ...
            <div class="s-card">
                <div class="s-head flex items-center justify-between">
                    <div>
                        <p class="text-[10px] font-black uppercase tracking-widest text-gray-400">{{ now()->format('F Y') }}</p>
                ...
                <div class="s-body">
                    ...
                    <div class="text-center mb-6">
                        <p class="text-5xl font-black text-gray-900">{{ $otBurnRate['used'] }}</p>
                        <p class="text-sm text-gray-400 mt-1">of <strong>{{ $otBurnRate['budget'] }}</strong> budgeted hours used</p>
                    </div>

                    <div class="gauge-track mb-2" style="height:14px;">
                        <div class="gauge-fill" style="width:{{ $otBurnRate['pct'] }}%;background:{{ $fillColor }};height:14px;"></div>
                    </div>
                    <div class="flex justify-between text-xs text-gray-400 mb-6">
                        <span>0h</span>
                        <span class="font-bold" style="color:{{ $fillColor }};">{{ $otBurnRate['pct'] }}% burned</span>
                        <span>{{ $otBurnRate['budget'] }}h</span>
                    </div>
                    ...
                    <div class="mt-5 border-t border-gray-50 pt-4">
                        ...
                        @foreach($topNDWorkers as $w)
                        <div class="flex justify-between items-center py-1.5 text-sm">
                            <span class="text-gray-700 font-medium truncate max-w-[130px]">{{ $w->name }}</span>
                            <div class="flex gap-3 text-xs font-bold shrink-0">
                                <span class="brand-teal">ND: {{ number_format($w->total_nd, 1) }}h</span>
                                <span class="brand-accent">OT: {{ number_format($w->total_ot, 1) }}h</span>
                            </div>
                        </div>
                        @endforeach
                    </div>
            ...
            <div class="s-card">
                ...
                <div class="s-body">
                    ...
                    <div class="flex flex-col items-center mb-6">
                        @php $pct13 = $thirteenthMonth['pct']; @endphp
                        <div class="relative w-36 h-36">
                            <svg class="w-full h-full -rotate-90" viewBox="0 0 36 36">
                                <circle cx="18" cy="18" r="15.9" fill="none" stroke="#e5e7eb" stroke-width="3"/>
                                <circle cx="18" cy="18" r="15.9" fill="none" stroke="#015581" stroke-width="3"
                                        stroke-dasharray="{{ $pct13 }}, 100"
                                        stroke-linecap="round"/>
                            </svg>
                            <div class="absolute inset-0 flex flex-col items-center justify-center">
                                <p class="text-2xl font-black brand-primary">{{ $pct13 }}%</p>
                                <p class="text-[10px] text-gray-400 font-bold">accrued</p>
                            </div>
                        </div>
                        <p class="text-sm text-gray-500 mt-2">
                            Month <strong>{{ $thirteenthMonth['months_elapsed'] }}</strong> of 12
                        </p>
                    </div>
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between items-center py-3 border-t border-gray-50">
                            <div>
                                ...
                                <p class="text-2xl font-black text-gray-900">₱{{ number_format($thirteenthMonth['accrued'], 2) }}</p>
                            </div>
                        ...
                        <div class="flex justify-between items-center py-3 border-t border-gray-50">
                            <div>
                                ...
                                <p class="text-xl font-black text-gray-500">₱{{ number_format($thirteenthMonth['full_liability'], 2) }}</p>
                            </div>
                        ...
                        @if($thirteenthMonth['full_liability'] === 0.0)
                    ...
                    <div class="mt-4 border-t border-gray-50 pt-4">
                        ...
                        <div class="flex gap-0.5">
                            ...
                            <div class="flex-1 rounded-sm h-5 flex items-center justify-center text-[9px] font-black
                                {{ $m <= $thirteenthMonth['months_elapsed'] ? 'brand-bg-p text-white' : 'bg-gray-100 text-gray-300' }}"
                                title="{{ \Carbon\Carbon::create(null, $m)->format('M') }}">
                                {{ \Carbon\Carbon::create(null, $m)->format('M')[0] }}
                            </div>
                            @endfor
                        </div>
                    </div>
