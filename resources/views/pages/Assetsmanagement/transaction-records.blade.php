<div class="max-w-7xl mx-auto py-8 px-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
            ...
            <p class="text-2xl font-extrabold text-blue-600">{{ number_format($this->summary['transfers']) }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
            <p class="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-1">Repairs</p>
            <p class="text-2xl font-extrabold text-amber-600">{{ number_format($this->summary['repairs']) }}</p>
        </div>
    </div>
    ...
    <div class="bg-white shadow-md rounded-xl overflow-hidden border border-gray-200">
        <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex justify-between items-center">
            ...
            <span class="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full font-medium">
                {{ $this->summary['count'] }} {{ $this->summary['count'] === 1 ? 'transaction' : 'transactions' }}
            </span>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse($this->transactions as $transaction)
                        ...
                        <tr class="hover:bg-amber-50/40 transition-colors">
                            <td class="px-6 py-4">
                                <span class="text-xs font-bold text-gray-400">#{{ $transaction->id }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm font-bold text-gray-900">{{ $transaction->asset?->sku ?? 'Unknown Asset' }}</div>
                                <div class="text-xs text-gray-400">
                                    {{ $transaction->asset?->itemType?->name ?? 'Unknown Item Type' }}
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold {{ $typeClasses }}">
                                    {{ ucfirst($transaction->type) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600">
                                <div>From: {{ $transaction->fromLocation?->name ?? 'N/A' }}</div>
                                <div>To: {{ $transaction->toLocation?->name ?? 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <p class="text-sm text-gray-700 font-medium">{{ $transaction->datetime?->format('M d, Y') }}</p>
                                <p class="text-xs text-gray-400">{{ $transaction->datetime?->format('h:i A') }}</p>
                            </td>
                            ...
                            <td colspan="6" class="px-6 py-14 text-center">
                                <div class="flex flex-col items-center text-gray-400">
                                    ...
                                    <p class="text-sm font-medium">
                                        {{ ($search || $type || $dateFrom || $dateTo) ? 'No transactions match your filters.' : 'No transaction records yet.' }}
                                    </p>
                                    @if($search || $type || $dateFrom || $dateTo)
                    ...
                    <div class="bg-white px-6 pt-6 pb-4 border-b border-gray-100">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 class="text-lg font-bold text-gray-900">Transaction #{{ $detail->id }}</h3>
                                    <p class="text-xs text-gray-400">{{ $detail->datetime?->format('F d, Y h:i A') }}</p>
                                </div>
                            </div>
                    ...
                    <div class="px-6 py-5 space-y-5">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="bg-gray-50 rounded-lg p-3 border border-gray-100">
                                ...
                                <p class="text-sm font-bold text-gray-800">{{ $detail->asset?->sku ?? 'Unknown Asset' }}</p>
                                <p class="text-xs text-gray-500">{{ $detail->asset?->itemType?->name ?? 'Unknown Item Type' }}</p>
                            </div>
                            <div class="bg-gray-50 rounded-lg p-3 border border-gray-100">
                                <p class="text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-0.5">Transaction Type</p>
                                <p class="text-sm font-bold text-gray-800">{{ ucfirst($detail->type) }}</p>
                            </div>
                            <div class="bg-gray-50 rounded-lg p-3 border border-gray-100">
                                <p class="text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-0.5">From Location</p>
                                <p class="text-sm font-bold text-gray-800">{{ $detail->fromLocation?->name ?? 'N/A' }}</p>
                            </div>
                            <div class="bg-gray-50 rounded-lg p-3 border border-gray-100">
                                <p class="text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-0.5">To Location</p>
                                <p class="text-sm font-bold text-gray-800">{{ $detail->toLocation?->name ?? 'N/A' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="rounded-lg border border-gray-200 bg-stone-50 p-4">
                            ...
                            <p class="text-sm text-gray-700">{{ $detail->notes ?: 'No notes recorded for this transaction.' }}</p>
                        </div>
                    </div>
