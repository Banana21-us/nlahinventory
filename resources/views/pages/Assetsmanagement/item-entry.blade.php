<div class="max-w-7xl mx-auto py-8 px-4">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div class="xl:col-span-2 bg-white shadow-md rounded-lg border border-gray-200 overflow-hidden" x-data="{ open: @entangle('showForm') }">
            <button
                type="button"
                @click="open = !open"
                ...
                <form wire:submit.prevent="save" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea
                            ...
                            class="block w-full rounded-md border border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm p-2"
                        ></textarea>
                        @error('desc') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>
        ...
        <div class="bg-white shadow-md rounded-lg border border-gray-200 overflow-hidden" x-data="{ open: @entangle('showCategoryForm') }">
            <button
                type="button"
                @click="open = !open"
                class="w-full flex items-center justify-between p-5 bg-white hover:bg-gray-50 transition-colors focus:outline-none"
                ...
                <form wire:submit.prevent="saveCategory" class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            class="block w-full rounded-md border border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm p-2"
                        ></textarea>
                        @error('category_desc') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>
        ...
        <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex flex-wrap gap-3 justify-between items-center">
            <div class="flex items-center gap-3">
                ...
                <span class="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full font-medium">
                    {{ $this->itemTypes->count() }} {{ $this->itemTypes->count() === 1 ? 'record' : 'records' }}
                </span>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse($this->itemTypes as $itemType)
                        <tr class="hover:bg-indigo-50/40 transition-colors">
                            <td class="px-6 py-4">
                                <div class="text-sm font-bold text-gray-900">{{ $itemType->name }}</div>
                                <div class="text-xs text-gray-400">Created {{ $itemType->created_at?->format('M d, Y') }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-indigo-100 text-indigo-700">
                                    {{ $itemType->category->name ?? 'Uncategorized' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600 max-w-md">
                                {{ $itemType->desc ?: 'No description' }}
                            </td>
                            <td class="px-6 py-4 text-sm font-semibold text-gray-700">
                                {{ $itemType->assets_count }}
                            </td>
                            <td class="px-6 py-4 text-right text-sm font-medium space-x-2">
                                <button
                                    wire:click="edit({{ $itemType->id }})"
                                ...
                                <button
                                    wire:click="confirmDelete({{ $itemType->id }})"
                            ...
                            <td colspan="5" class="px-6 py-14 text-center">
                                <div class="flex flex-col items-center text-gray-400">
                                    ...
                                    <p class="text-sm font-medium">
                                        {{ $search ? 'No item types match your search.' : 'No item types yet.' }}
                                    </p>
                                    <p class="text-xs mt-1">
                                        {{ $search ? 'Try a different keyword.' : 'Use the form above to create your first item type.' }}
                                    </p>
                                </div>
                            </td>
                        </tr>
