            ...
            <div class="bg-gradient-to-r from-sky-600 to-sky-700 px-6 py-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-lg font-bold">{{ $isEditing ? 'Edit Asset' : 'Register New Asset' }}</h3>
            ...
            <form wire:submit.prevent="{{ $isEditing ? 'update' : 'save' }}" class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        @error('condition_status') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>
                ...
                <div class="flex justify-end items-center gap-3 pt-4 border-t border-gray-100 mt-4">
                    ...
                    <button type="submit" class="bg-sky-600 hover:bg-sky-700 text-white text-sm font-bold py-2 px-6 rounded shadow-md transition-all active:scale-95 flex items-center gap-2">
                        <span wire:loading.remove wire:target="save,update">{{ $isEditing ? 'Update Asset' : 'Register Asset' }}</span>
    ...
    <div class="mt-8 bg-white shadow-md rounded-lg overflow-hidden border border-gray-200">
        <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex flex-wrap gap-3 justify-between items-center">
            <div class="flex items-center gap-3">
                ...
                <span class="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full font-medium">{{ $assets->total() }} {{ $assets->total() === 1 ? 'record' : 'records' }}</span>
            </div>
        </div>
            ...
            <table class="w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse($assets as $asset)
                        ...
                        @endphp
                        <tr class="hover:bg-sky-50/40 transition-colors cursor-pointer" wire:click="showDetails({{ $asset->id }})">
                            ...
                            <td class="px-6 py-4"><span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold {{ $statusClasses }}">{{ ucfirst(str_replace('_', ' ', $asset->status)) }}</span></td>
                            <td class="px-6 py-4"><span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold {{ $conditionClasses }}">{{ ucfirst($asset->condition_status) }}</span></td>
                            <td class="px-6 py-4">
                                @if($isAssigned)
                            ...
                            <td class="px-6 py-4 text-right text-sm font-medium space-x-2" wire:click.stop>
                                <button wire:click="edit({{ $asset->id }})" class="rounded-md bg-sky-50 px-2.5 py-1.5 text-sm font-semibold text-sky-700 shadow-sm hover:bg-sky-100 transition-colors">Edit</button>
                                <button wire:click="confirmDelete({{ $asset->id }})" class="text-red-500 hover:text-red-700 font-semibold transition-colors">Delete</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-14 text-center">
                                <div class="flex flex-col items-center text-gray-400">
                                    ...
                                    <p class="text-sm font-medium">{{ $search ? 'No assets match your search.' : 'No asset records yet.' }}</p>
                                    <p class="text-xs mt-1">{{ $search ? 'Try a different keyword.' : 'Click "Add Asset" above to get started.' }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="bg-gray-50 rounded-lg p-4 flex items-center justify-center">
                                @if($selectedAsset->image && Storage::disk('public')->exists($selectedAsset->image))
                                    <img src="{{ Storage::url($selectedAsset->image) }}" class="w-full max-w-md object-cover rounded-lg shadow-md">
                                @else
                            ...
                            <div class="space-y-4">
                                <div class="border-b pb-2">
                                    ...
                                    <p class="text-xl font-bold text-gray-900">{{ $selectedAsset->asset_code }}</p>
                                </div>

                                <div class="border-b pb-2">
                                    <p class="text-xs font-bold uppercase tracking-wide text-gray-400">Asset Name</p>
                                    <p class="text-lg font-semibold text-gray-800">{{ $selectedAsset->name }}</p>
                                </div>

                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <p class="text-xs font-bold uppercase tracking-wide text-gray-400">Brand</p>
                                        <p class="text-gray-700">{{ $selectedAsset->brand ?: 'N/A' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-xs font-bold uppercase tracking-wide text-gray-400">Model</p>
                                        <p class="text-gray-700">{{ $selectedAsset->model ?: 'N/A' }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p class="text-gray-700">{{ $selectedAsset->category ?: 'N/A' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-xs font-bold uppercase tracking-wide text-gray-400">Serial Number</p>
                                        <p class="text-gray-700">{{ $selectedAsset->serial_number ?: 'N/A' }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p class="text-gray-700">{{ $selectedAsset->purchase_date ? date('F d, Y', strtotime($selectedAsset->purchase_date)) : 'N/A' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-xs font-bold uppercase tracking-wide text-gray-400">Purchase Cost</p>
                                        <p class="text-gray-700">₱{{ number_format($selectedAsset->purchase_cost, 2) }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold {{ $statusColor }}">{{ ucfirst(str_replace('_', ' ', $selectedAsset->status)) }}</span>
                                    </div>
                                    <div>
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold {{ $conditionColor }}">{{ ucfirst($selectedAsset->condition_status) }}</span>
                                    </div>
                                </div>
                                    ...
                                    @if($isAssigned)
                                        <div class="mt-2 space-y-2">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span class="font-medium text-gray-800">{{ $selectedAsset->department->name ?? 'N/A' }}</span>
                                            </div>
                                            <div class="flex items-center gap-2">
                                                <span class="text-sm text-gray-500">Location:</span>
                                                <span class="font-medium text-gray-800">{{ $selectedAsset->location->name ?? 'N/A' }}</span>
                                            </div>
                                        </div>
                                    @else
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-600">Unassigned</span>
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-600 mt-1">{{ $selectedAsset->notes ?: 'No notes available.' }}</p>
                                </div>

                                <div class="pt-2 text-xs text-gray-400 border-t">
                                    <p>Created: {{ $selectedAsset->created_at ? date('F d, Y h:i A', strtotime($selectedAsset->created_at)) : 'N/A' }}</p>
                                    <p>Last Updated: {{ $selectedAsset->updated_at ? date('F d, Y h:i A', strtotime($selectedAsset->updated_at)) : 'N/A' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    @if (session()->has('message'))
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)" x-transition:enter="transform ease-out duration-300 transition" x-transition:enter-start="translate-y-2 opacity-0 sm:translate-y-0 sm:translate-x-2" x-transition:enter-end="translate-y-0 opacity-100 sm:translate-x-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed top-5 right-5 z-[60] w-full max-w-sm overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-black/5">
            <div class="p-4 flex items-start gap-3">
                ...
                <div class="flex-1 pt-0.5"><p class="text-sm font-semibold text-gray-900">Success!</p><p class="mt-0.5 text-sm text-gray-500">{{ session('message') }}</p></div>
